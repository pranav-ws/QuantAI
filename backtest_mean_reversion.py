...
Backtests the Mean Reversion strategy on a single stock.

Unlike the ML backtests (src/backtest.py, backtest_ensemble.py) which
flip position on the very next bar, this strategy HOLDS a position
until one of three exit conditions fires:

    1. REVERT — price has snapped back near the 20-day mean (take profit)
    2. STOP   — price kept falling and hit the ATR-based stop loss
    3. TIME   — the reversion never happened within max_holding_days

Because this is a rule-based strategy (no training), it's backtested
across the FULL history rather than a held-out test split — there's
no lookahead risk since nothing was fitted to the data.
"""
import os
...
RISK_PER_TRADE  = 0.95   # use 95% of capital per position, same convention as backtest.py


def run_mean_reversion_backtest(ticker, initial_capital=INITIAL_CAPITAL, strategy=None):
    strategy = strategy or MeanReversionStrategy()

    print(f"\n{'='*58}")
    print(f"  QuantAI Mean Reversion Backtest — {ticker}")
    ...
    for date, row in df.iterrows():
        ...
        if position > 0:
            ...
            if hit_stop or reverted or timed_out:
                ...
                reason = 'STOP' if hit_stop else ('REVERT' if reverted else 'TIME')
                trades.append({
        ...
        # ── Look for new entry ────────────────────────────
        if position == 0 and row['MR_Signal'] == 'BUY' and \
           row['MR_Confidence'] >= strategy.min_confidence:
            position    = int(capital * RISK_PER_TRADE / price)
            if position > 0:
                entry_price = price
                entry_date  = date
                stop_loss   = price - strategy.stop_loss_atr_mult * row['ATR_14']
...
if __name__ == '__main__':
    equity_df, trade_df, signal_df, metrics = run_mean_reversion_backtest(TICKER)
    ...
    ax1.plot(equity_df.index, equity_norm, color='#4ecdc4', linewidth=1.6,
              label='Mean Reversion Strategy', zorder=3)
    ...
    if not trade_df.empty:
        ...
        ax1.scatter(trade_df['entry_date'], entry_vals, marker='^', color='#22c55e',
                    s=60, zorder=4, label='Entry (oversold bounce)')

    ax1.axhline(100, color='white', linewidth=0.5, linestyle=':', alpha=0.4)
    ax1.set_title(f'Mean Reversion vs Buy & Hold — {TICKER}', color='white', fontsize=12, pad=12)
    ...
    plt.savefig('models/mean_reversion_backtest.png', dpi=150, bbox_inches='tight', facecolor='#0d0d1a')
    plt.show()
    print("📊 Chart saved → models/mean_reversion_backtest.png")