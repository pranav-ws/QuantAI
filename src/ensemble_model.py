...
def _load_models(ticker):
    ...
    # ── Transformer (train_transformer_all.py) ───────────
    # This was completely missing before — no matter how many times you
    # trained the Transformer, the ensemble never looked for its saved
    # file, so it always showed "not trained" regardless of training.
    tr_path        = os.path.join(MODELS_DIR, f'{ticker}_transformer_model.pkl')
    tr_scaler_path = os.path.join(MODELS_DIR, f'{ticker}_transformer_scaler.pkl')
    if os.path.exists(tr_path) and os.path.exists(tr_scaler_path):
        try:
            models['Transformer'] = (joblib.load(tr_path), joblib.load(tr_scaler_path))
        except Exception:
            pass

    # ── RL Agent (train_rl_agent.py) ──────────────────────
    # Also completely missing before, same bug as Transformer above.
    rl_path = os.path.join(MODELS_DIR, f'{ticker}_rl_agent.pkl')
    if os.path.exists(rl_path):
        try:
            from src.rl_agent import load_agent
    ...
    for name, model_obj in models.items():
        try:
            if name in ('SeqNN', 'LSTM'):
                ...
                from src.transformer_model import predict_transformer, SEQUENCE_LEN as TR_SEQ_LEN
                if len(df) >= TR_SEQ_LEN:
                    model, scaler = model_obj
                    confidences[name] = predict_transformer(model, scaler, df)
            elif name == 'RL':
                from src.rl_agent import get_rl_signal
                _, conf, _ = get_rl_signal(ticker, df)
                if conf > 0:
...
def get_model_agreement(individual_confs):
    ...
    ML_NAMES = {'RF', 'XGBoost', 'SeqNN', 'LSTM', 'Transformer', 'RL', 'TFT'}
