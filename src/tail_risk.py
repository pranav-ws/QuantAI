...
class TailRiskMonitor:
    ...
    def scan_from_db(self, period_days: int = 252) -> TailRiskReport:
        """
        Loads stock data from local SQLite DB and runs scan().
        Fast path: DB only — no yfinance fallback (avoids 60s timeout).
        Uses last 60 days and up to 20 stocks for speed.
        """
        from src.data_collector import STOCK_UNIVERSE

        db_path = os.path.join(os.path.dirname(__file__), "..", "data", "quantai.db")
        # ^ was os.path.join('data', 'quantai.db') — CWD-relative, so if
        # uvicorn was launched from any working directory other than the
        # project root (e.g. from inside src/, or a different shell cwd
        # from Task Scheduler), this raised "Price database not found" even
        # though the DB existed, because it was looking in the wrong place.
        # Now file-relative like database.py and auth.py, so it always
        # resolves to <project_root>/data/quantai.db regardless of cwd.
        price_dict  = {}
        volume_dict = {}

        # Strict DB-only loading — no yfinance fallback in API context
        if not os.path.exists(db_path):
            raise FileNotFoundError(
                "Price database not found. Run python pipeline.py first."
            )

        # Use 60 days for speed (enough for all 6 TRI components)
        fast_days = min(period_days, 60)

        try:
            conn = sqlite3.connect(db_path)
            # Load up to 20 stocks that have the most data (fastest query)
            tickers_with_data = pd.read_sql_query(
        ...
        if not tickers_with_data:
            raise RuntimeError(
                "Not enough price data in DB. "
                "Run python pipeline.py to download stock data."
            )
        ...
        for ticker in tickers_with_data:
            try:
                df = pd.read_sql_query(
                    ...
                    conn, params=(ticker, fast_days + 5)
                )
                conn_ok = True
            except Exception:
                continue
        ...
        if len(price_dict) < 3:
            raise RuntimeError(
                f"Only {len(price_dict)} stocks with sufficient data. "
