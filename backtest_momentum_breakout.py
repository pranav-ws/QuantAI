"""
backtest_momentum_breakout.py

Backtests the Momentum Breakout strategy on a single stock.

Exit logic is deliberately different from backtest_mean_reversion.py:
mean reversion wants to take profit FAST once price snaps back to the
mean, but momentum wants to let a confirmed trend run as long as
possible. So instead of a fixed take-profit, this uses a CHANDELIER
TRAILING STOP — the stop only ever ratchets upward, tracking the
highest close reached since entry minus a multiple of ATR. The
position is only closed when:

    1. TRAIL_STOP — price pulls back far enough from its peak to hit
                     the trailing stop (the trend may be over)
    2. STOP       — price never even got going and hit the initial
                     hard stop loss
    3. TIME       — max holding period reached (sideways stagnation)

Like backtest_mean_reversion.py, this is a rule-based strategy (no
training), so it's backtested across the full history.
"""
import os
...
def run_momentum_breakout_backtest(ticker, initial_capital=INITIAL_CAPITAL, strategy=None):
    strategy = strategy or MomentumBreakoutStrategy()

    print(f"\n{'='*58}")
    print(f"  QuantAI Momentum Breakout Backtest — {ticker}")
    ...
    for date, row in df.iterrows():
        ...
        # ── Look for new entry ────────────────────────────
        if position == 0 and row['BO_Signal'] == 'BUY' and \
           row['BO_Confidence'] >= strategy.min_confidence:
...
if __name__ == '__main__':
    equity_df, trade_df, signal_df, metrics = run_momentum_breakout_backtest(TICKER)
    ...
    ax1.plot(equity_df.index, equity_norm, color='#fbbf24', linewidth=1.6,
              label='Momentum Breakout Strategy', zorder=3)
    ...
    if not trade_df.empty:
        ...
        ax1.scatter(trade_df['entry_date'], entry_vals, marker='^', color='#22c55e',
                    s=60, zorder=4, label='Entry (breakout)')

    ax1.axhline(100, color='white', linewidth=0.5, linestyle=':', alpha=0.4)
    ax1.set_title(f'Momentum Breakout vs Buy & Hold — {TICKER}', color='white', fontsize=12, pad=12)
    ...
    plt.savefig('models/momentum_breakout_backtest.png', dpi=150, bbox_inches='tight', facecolor='#0d0d1a')
    plt.show()
    print("📊 Chart saved → models/momentum_breakout_backtest.png")